...
#endif
{
  ...
  if (quantizer->state->flag4[var]) {
    ...
    if (a[var] != 0 && (fabs(x[cf3]) > 1e-10) && !quantizer->state->flag3[var] && !quantizer->state->flag2[var]) {
      double coeff[3];
      coeff[0] = a[var] * a[var] * a[var] * q[cf0] + a[var] * a[var] * u0[var] + a[var] * u1[var] + 2 * u2[var];
      coeff[1] = a[var] * a[var] * a[var] * q[cf1] + a[var] * a[var] * u1[var] + a[var] * 2 * u2[var];
...
#ifdef QSS_PARALLEL
void LIQSS3_PAR_nextTime(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
#else
void LIQSS3_nextTime(QA_quantizer quantizer, int var, double t, double *nTime, double *x, double *lqu)
#endif
{
  int cf3 = var * 4 + 3;
  if (x[cf3] == 0) {
...
#endif
{
  ...
  double *dq = quantizer->state->dq;
  quantizer->state->flag3[var] = 0;
  elapsed = quantizer->state->lSimTime->time - quantizer->state->lSimTime->tq[var];
  ...
  if (quantizer->state->a[var] < -1e-30) {
    if (x[cf3] > 0) {
      dx = a[var] * a[var] * a[var] * (q[cf0] + lqu[var]) + a[var] * a[var] * u0[var] + a[var] * u1[var] + 2 * u2[var];
      if (dx >= 0) {
        dq[var] = lqu[var];
      } else {
        dq[var] = -2 * u2[var] / a[var] / a[var] / a[var] - u1[var] / a[var] / a[var] - u0[var] / a[var] - q[cf0];
    ...
    if (q[cf1] * x[cf1] < 0 && !quantizer->state->flag4[var] && !quantizer->state->flag2[var] && !quantizer->state->flag3[var]) {
      if (q[cf1] < 0) {
        dq[var] = quantizer->state->qAux[var] - q[cf0] - fabs(quantizer->state->lquOld[var]) * 0.1;
      } else {
        dq[var] = quantizer->state->qAux[var] - q[cf0] + fabs(quantizer->state->lquOld[var]) * 0.1;
      }
      ...
      if (fabs(-2 * u2[var] / a[var] / a[var] / a[var] - u1[var] / a[var] / a[var] - u0[var] / a[var] - q[cf0]) < 3 * lqu[var]) {
        dq[var] = -2 * u2[var] / a[var] / a[var] / a[var] - u1[var] / a[var] / a[var] - u0[var] / a[var] - q[cf0];
        quantizer->state->flag3[var] = 1;
      }
    }
