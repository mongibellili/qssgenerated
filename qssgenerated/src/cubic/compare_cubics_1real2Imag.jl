using LinearAlgebra
#=
compare case of 1 real 2 imag
cubics based on the depressed cubic  x^3+px+q=0
cubic2: res1=c-p/(3c)+S    where   c=cbrt(-q/2+m) ;m=sqrt((q*q/4)+p*p*p/27)
cubic3: res1=cbrt(-q/2+m)+cbrt(-q/2-m)+S where     m=sqrt((q*q/4)+p*p*p/27)
cubic4: res1=α-p/(3α)+S    where α=cbrt(quadratic(1.0,q,-(p^3)/27))
cubic5: res1 =  P0 + p/(3*P0) + S  where  P0 = q < 0 ? cbrt(0.5 * (-q (+/-) √Δ))  ;Δ=q*q-4*p*p*p/27
=#
  ...
  if a == 0.0
    ...
    if d == 0.0
      ...
      if 4p*p*p+27q*q>0  #2Imag 1 real
        ...
        c=cbrt(-q/2+m)   # this one cbrt is faster the cubic3 which has 2 cbrts
        res1=c-p/(3c)+S
        res2=Inf
        res3=Inf
...
function cubic5(a::F, b::F, c::F, d::F) where F <: AbstractFloat
   ...
   #=  m = b < c ? b : c
    ...
    if Δ > 4eps(F) # one real root and two complex roots
      P0 = yₙ < 0 ? cbrt(0.5 * (-yₙ + √Δ)) : cbrt(0.5 * (-yₙ - √Δ))
      #q = δ² / P
      res1 = xₙ + P0 + δ² / P0
      res2=Inf
      res3=Inf
...
@btime cubic2(1.33,2.0,1.0,6.0) #(-2.1385740543503533, Inf, Inf)
@btime cubic3(1.33,2.0,1.0,6.0) #(-2.138574501422444, Inf, Inf)
@btime cubic4(1.33,2.0,1.0,6.0) # (-2.1385745013176427, Inf, Inf)
@btime cubic5(1.33,2.0,1.0,6.0)#(-2.1385745013176427, Inf, Inf)
